...
class Solution:
    ...
    * `lcp[i][j]` is equal to the length of the **longest common prefix** between the
    ...
    earlier in the alphabet than the corresponding letter in `b`. For example, `"aabd"`
    is lexicographically smaller than `"aaca"` because the first position they differ is
    at the third letter, and `'b'` comes before `'c'`."""
